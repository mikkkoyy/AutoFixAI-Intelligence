...
class MainWindow(QMainWindow):
    ...
    def monitor_page(self):
        ...
        lifecycle = QFrame()
        lifecycle.setObjectName(
            "sectionCard"
        )

        lifecycle_layout = QGridLayout(
            lifecycle
        )
        ...
        lifecycle_layout.setHorizontalSpacing(
            8
        )
        ...
        for column, (number, name) in enumerate(
            phases
        ):
            phase = QFrame()
            ...
            phase_layout.setContentsMargins(
                10,
                10,
                10,
                10,
    ...
    def update_lifecycle(self, status):
        """
        Update the visual lifecycle based on backend status/event names.

        This intentionally uses the existing backend status/event data.
